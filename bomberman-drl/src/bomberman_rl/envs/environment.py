...
class GenericWorld:
    ...
    def place_bombs_single_coin_fixed(self):
        #WALL = -1
        #FREE = 0
        #CRATE = 1
        if not [coin for coin in self.coins if coin.collectable==True]:


            cur_agent=self.agents[0]
            limit_x=16
            limit_y=16
           
           
            bomb_xy=[(bomb.x,bomb.y) for bomb in self.bombs]
            blast_coords=[bomb.get_blast_coords(self.arena) for bomb in self.bombs]
            
            (x_agent,y_agent)=(cur_agent.x,cur_agent.y)
            possible_coin_positions=[]

            q=deque()
            q.append([x_agent,y_agent,0])
            path_list=[[x_agent,y_agent]]
            distanz=0
            while q:
                cur_pos=q.popleft()
                x=cur_pos[0]
                ...
                if self.arena[x,y+1]==0 and [x,y+1] not in path_list:
                    q.append([x,y+1,d+1])
                ...
                if self.arena[x+1,y]==0 and [x+1,y] not in path_list:
                    q.append([x+1,y,d+1])
                ...
                if d<=10:
                   possible_coin_positions.append(Coin((x,y),True))
                    


            random.shuffle(possible_coin_positions)
            self.coins.append(possible_coin_positions[0])




        if not self.explosions and not self.bombs:
            
            cur_agent=self.agents[0]
            limit_x=16
            limit_y=16
            cur_coin=[coin for coin in self.coins if coin.collectable==True][0]
            explosion_xy=[(explosion.x,explosion.y) for explosion in self.explosions if explosion.stage==0 and explosion.timer==2]
            bomb_xy=[(bomb.x,bomb.y) for bomb in self.bombs]
            blast_coords=[bomb.get_blast_coords(self.arena) for bomb in self.bombs]
            (x_coin,y_coin)=(cur_coin.x,cur_coin.y)
            (x_agent,y_agent)=(cur_agent.x,cur_agent.y)
            ...
            while q:
                ...
                if self.arena[x,y+1]==0 and [x,y+1] not in path_list and (x,y+1) not in explosion_xy and (x,y+1) not in blast_coords:
                    q.append([x,y+1,d+1])
                    path_list.append([x,y+1])
                ...
                if self.arena[x+1,y]==0 and [x+1,y] not in path_list and (x+1,y) not in explosion_xy and (x+1,y) not in blast_coords:
                    q.append([x+1,y,d+1])
                    path_list.append([x+1,y])
            ...
            if distanz>3:
                for i in range(1,limit_x):
                   for j in range(1,limit_y):
                       if self.arena[i,j]==0 and (i,j) not in explosion_xy and (i,j) not in bomb_xy:
                            cur_bomb=Bomb((i, j),cur_agent,s.BOMB_TIMER,s.BOMB_POWER,cur_agent.bomb_sprite)
                            bomb_blast=cur_bomb.get_blast_coords(self.arena)
                            if (x_agent,y_agent) not in bomb_blast and (x_coin,y_coin) not in bomb_blast:
                                    q=deque()
                                    q.append([x_agent,y_agent,0])
                                    path_list=[[x_agent,y_agent]]
                                    distanz_new=0
                                    while q:
                                        ...
                                        if self.arena[x,y+1]==0 and [x,y+1] not in path_list and (x,y+1) not in explosion_xy and (x,y+1) not in blast_coords and (x,y+1) not in bomb_blast:
                                            q.append([x,y+1,d+1])
                                            path_list.append([x,y+1])
                                        ...
                                        if self.arena[x+1,y]==0 and [x+1,y] not in path_list and (x+1,y) not in explosion_xy and (x+1,y) not in blast_coords and (x+1,y) not in bomb_blast:
                                            q.append([x+1,y,d+1])
                                            path_list.append([x+1,y])
                                    ...
                                    if distanz_new>distanz:
                                        cur_bomb.timer-=1
                                        possible_bomb_positions.append(cur_bomb)

            random.shuffle(possible_bomb_positions)
            if possible_bomb_positions:
                self.bombs.append(possible_bomb_positions[0])



    

    def place_bombs_bomb_escape(self):
        #WALL = -1
        #FREE = 0
        #CRATE = 1
        window_length=4
        max_concurrent_bombs=1
        time_limit_escape=10
        cur_agent=self.agents[0]
        limit_x=16
        limit_y=16
       
        explosion_xy=[(explosion.x,explosion.y) for explosion in self.explosions if explosion.stage==0]
        bomb_xy=[(bomb.x,bomb.y) for bomb in self.bombs]
        (x,y)=(cur_agent.x,cur_agent.y)
        possible_bomb_positions=[]
        cur_bombs_in_window=0
        ...
        for i in range(0,window_length):
            for j in range(0,window_length):
                ...
                cur_x=x+i
                cur_y=y+j
                #es gibt explosionen mit Stage 1 muesssen nicht berucksichtuigt werden
                if cur_x<limit_x and cur_y<limit_y and self.arena[cur_x,cur_y]==0 and (cur_x,cur_y) not in explosion_xy and (cur_x,cur_y) not in bomb_xy:
                    possible_bomb_positions.append((cur_x,cur_y))

                if cur_x<limit_x and cur_y<limit_y and (cur_x,cur_y) in bomb_xy:
                    cur_bombs_in_window+=1

                cur_x=x-i
                cur_y=y-j
                if (i>0 or j>0) and cur_x>0 and cur_y>0 and self.arena[cur_x,cur_y]==0 and (cur_x,cur_y) not in explosion_xy and (cur_x,cur_y) not in bomb_xy:
                    possible_bomb_positions.append((cur_x,cur_y))
                ...
                cur_x=x+i
                cur_y=y-j
                if (i>0 or j>0) and cur_x<limit_x and cur_y>0 and self.arena[cur_x,cur_y]==0 and (cur_x,cur_y) not in explosion_xy and (cur_x,cur_y) not in bomb_xy:
                    possible_bomb_positions.append((cur_x,cur_y))
                ...
                cur_x=x-i
                cur_y=y+j
                if (i>0 or j>0) and cur_x>0 and cur_y<limit_y and self.arena[cur_x,cur_y]==0 and (cur_x,cur_y) not in explosion_xy and (cur_x,cur_y) not in bomb_xy:
                    possible_bomb_positions.append((cur_x,cur_y))
        ...
        max_concurrent_bombs-=cur_bombs_in_window
        ...
        for explosion in self.explosions:
            ...
            for t in range(0,explosion.timer):
               timestamp_dead_marker[t,cur_x,cur_y]=4

        for bomb in self.bombs:
            bomb_blast=bomb.get_blast_coords(self.arena)
            cur_bomb_x=bomb.x
            cur_bomb_y=bomb.y
            for (cur_x,cur_y) in bomb_blast:


               for t in range(bomb.timer,bomb.timer+s.EXPLOSION_TIMER):
                   timestamp_dead_marker[t,cur_x,cur_y]=4

               if cur_x==cur_bomb_x and cur_bomb_y==cur_y:
                     for t in range(0,bomb.timer):
                            timestamp_dead_marker[t,cur_x,cur_y]=4
        ...
        for (cur_bomb_x,cur_bomb_y) in possible_bomb_positions:
            if max_concurrent_bombs<=0:
                break
            cur_bomb=Bomb((cur_bomb_x, cur_bomb_y),cur_agent,s.BOMB_TIMER,s.BOMB_POWER,cur_agent.bomb_sprite)
            bomb_blast=cur_bomb.get_blast_coords(self.arena)
            remember_values=[]
            for (cur_x,cur_y) in bomb_blast:

                for t in range(cur_bomb.timer+s.EXPLOSION_TIMER,time_limit_escape):
                   if timestamp_dead_marker[t,cur_x,cur_y]==2:
                       remember_values.append((t,cur_x,cur_y,timestamp_dead_marker[t,cur_x,cur_y]))
                       timestamp_dead_marker[t,cur_x,cur_y]=0
                   elif timestamp_dead_marker[t,cur_x,cur_y]==3:
                        remember_values.append((t,cur_x,cur_y,timestamp_dead_marker[t,cur_x,cur_y]))
                        timestamp_dead_marker[t,cur_x,cur_y]=1
                   else:
                       break

                for t in range(cur_bomb.timer,cur_bomb.timer+s.EXPLOSION_TIMER):
                       remember_values.append((t,cur_x,cur_y,timestamp_dead_marker[t,cur_x,cur_y]))
                       timestamp_dead_marker[t,cur_x,cur_y]=4

                if cur_x==cur_bomb_x and cur_bomb_y==cur_y:
                     for t in range(0,cur_bomb.timer):
                           remember_values.append((t,cur_x,cur_y,timestamp_dead_marker[t,cur_x,cur_y]))
                           timestamp_dead_marker[t,cur_x,cur_y]=4
                    ...
                    for t in range(0,cur_bomb.timer):
                           remember_values.append((t,cur_x,cur_y,timestamp_dead_marker[t,cur_x,cur_y]))
                           if timestamp_dead_marker[t,cur_x,cur_y]==2:
                                timestamp_dead_marker[t,cur_x,cur_y]=3
                           else:
                                timestamp_dead_marker[t,cur_x,cur_y]=max(1,timestamp_dead_marker[t,cur_x,cur_y])
                          

            if find_rescue_route(timestamp_dead_marker,x,y,0,{},time_limit_escape)==1:
                cur_bomb.timer-=1
                self.bombs.append(cur_bomb)
                max_concurrent_bombs-=1
            else:
                for (t,pos1,pos2,value) in remember_values:
                    timestamp_dead_marker[t,pos1,pos2]=value
    ...
    def do_step(self, env_user_action):
        ...
        if self.scenario =="bomb-escape":
            self.place_bombs_bomb_escape()
        elif self.scenario =="single-coin-fixed":
            self.place_bombs_single_coin_fixed()
...
class BombeRLeWorld(GenericWorld):
    ...
    def get_state_for_agent(self, agent: Agent):
        ...
        state = {
            ...
            "others": [
                other.get_state() for other in self.active_agents if other is not agent # in self.active_agents if other is not agent
            ],
