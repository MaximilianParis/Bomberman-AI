...
def Transform_State(state):
        ...
        #berechne 2. Staterepresentation fuer Muenzen
        for i in range(x_old - window_length, x_old + window_length+1):
           for j in range(y_old - window_length, y_old + window_length+1):
               distanz = abs(i - x_old) + abs(j - y_old)  
               if distanz < window_length:
                   if i>=0 and i<limit_x and j>=0 and j<limit_y:
                       if((i,j)==(x_coin,y_coin)):
                           state_representation_list.append(1.0)
...
class Model():
    def __init__(self,epsilon_greedy=True, load=False, path=Path(__file__).parent / "model_coin_collect.pt"):
        self.initial=True
        ...
        self.memory = dict([(STEPPED_IN_BOMB_NEG,(ReplayMemory(5_000),300)),(STEPPED_IN_EXPLOSION,(ReplayMemory(5_000),300)),
                            (STEPPED_TOWARDS_COIN_POS,(ReplayMemory(15000),800)), (STEPPED_TOWARDS_COIN_NEG,(ReplayMemory(15000),800))
                            ,(STEPPED_IN_BLOCKED_FIELD,(ReplayMemory(7000),300)),(COLLECTED_COIN,(ReplayMemory(5000),100))]) 

        self.memory_state_action_pairs=dict([
                            (STEPPED_TOWARDS_COIN_POS,([],[]))]) 


        self.cnt_event=dict([(STEPPED_IN_BOMB_NEG,0),(STEPPED_IN_EXPLOSION,0),(STEPPED_TOWARDS_COIN_POS,0),(STEPPED_TOWARDS_COIN_NEG,0)
                            ,(STEPPED_IN_BLOCKED_FIELD,0),(COLLECTED_COIN,0)]) 
        self.cnt=0
        ...
        if self.policy_net is not None:
          ...
          #in Abhaengigkeit der Performance des Agenten in den Kategorien
            ...
            for mem in self.memory.items():
               ...
               #fuelle Batch mit Transitionen bezueglich Haufigkeit, Wichtigkeit und Verhealtnis positiven zu negativen Ereignissen
                if (mem[0]==STEPPED_TOWARDS_COIN_POS or mem[0]==STEPPED_TOWARDS_COIN_NEG) and not self.initial and cond:
                    factor=self.cnt_event[STEPPED_TOWARDS_COIN_NEG]/(self.cnt_event[STEPPED_TOWARDS_COIN_POS]+self.cnt_event[STEPPED_TOWARDS_COIN_NEG]+0.00000001)
                    ...
                    self.memory[mem[0]]=(mem[1][0],int(factor*2000))
                     ...
                     mem=(mem[0],(mem[1][0],self.cnt_event[STEPPED_IN_EXPLOSION]*10+100))
                     self.memory[mem[0]]=(mem[1][0],self.cnt_event[STEPPED_IN_EXPLOSION]*10+100)
                     ...
                     self.memory[mem[0]]=(mem[1][0],self.cnt_event[STEPPED_IN_BLOCKED_FIELD]+100)

                elif (mem[0]==STEPPED_IN_BOMB_NEG) and not self.initial and cond:
                     mem=(mem[0],(mem[1][0],self.cnt_event[STEPPED_IN_BOMB_NEG]*4+100))
                     self.memory[mem[0]]=(mem[1][0],self.cnt_event[STEPPED_IN_BOMB_NEG]*4+100)
            ...
            if transitions:
                #wir haben provisorisch die Implementierung so angepasst damit kein 
                #DQN benutzt wird
                #Das Netz wird hier so optimiert sodass es den reward fuer Zustandaktionspaare lernt
    ...
    def experience(self, old_state, action, new_state, reward,custom_events):
        ...
        old_state_transformed=Transform_State(old_state)
        if new_state is not None:
            new_state_transformed=Transform_State(new_state)
       
        if self.policy_net is None:
            self.lazy_init(old_state_transformed)
        for event in custom_events:
            if event in Replay_Kategorie_List:
                #speichere Zustandaktionspaar
                ...
                if event==STEPPED_TOWARDS_COIN_POS:
