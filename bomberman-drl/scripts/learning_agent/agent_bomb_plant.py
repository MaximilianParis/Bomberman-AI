...
from .learning_bomb_plant import Model
...
def compute_timestamp_dead_marker(opponents,bombs,explosions,crates,walls,cur_bomb_x,cur_bomb_y,time_limit_escape=10):
    ...
    #betrachten hier im Vergleich zu der anderen Version in agent.py die Zeitschritte ab 1 und nicht 0.
    #Zusaetzlich wird betrachtet wenn der Agent selbst eine Bombe legt. Das wird benutzt um zu pruefen ob
    #der Agent nach dem Legen der Bombe entkommen kann, deswegen muss die gelegte Bombe und das Abhauen danach beruecksichtigt werden.
    #Sonst funktioniert das wie in agent.py
...
class Agent_Bomb_Plant(LearningAgent):
    """
    ...
    def act(self, state,x_target,y_target,target, **kwargs) -> int:
        """
        ...
        #return action
        action=self.q_learning.act(state,x_target,y_target,target)[0].item()
        return action
    ...
    def game_events_occurred(
        ...
        reward=0
        if cond:
            custom_events = self._custom_events(old_state,self_action, new_state,target_x,target_y,target)
            reward = self._shape_reward(events + custom_events)
            self.q_learning.experience(old_state,self_action, new_state, reward,custom_events,target_x,target_y,target)
        if self.train:
    ...
    def save_supervised_state_action_pairs_bomb_plant(self):
        return self.q_learning.memory_state_action_pairs.items()

    def _custom_events(self, old_state,action, new_state,target_x,target_y,target):
        """
        ...
        bombs_left=old_state["self_info"]["bombs_left"]
        opponents_old=old_state["opponents_pos"]
        bomb_exploison_radius=np.zeros((17,17))
       ...
       #Agent soll fuer Bombenposition nicht traeniert werden, macht anderer Agent
        if bomb_exploison_radius[x_old,y_old]==0:
            ...
            if new_state is not None:
                ...
                #berechnet ob Agent abhauen kann nach dem Legen der Bombe oder nicht
                Bomb_Exlpoison_Radius_Local=Compute_Bomb_Exlpoison_Radius_Local(walls_old,x_old,y_old)
                timestamp_dead_marker=compute_timestamp_dead_marker(old_state["opponents_pos"],bombs_old,explosions_old,crates_old,walls_old,x_old,y_old)
                memo={}
                limit=5
                find_rescue_route(timestamp_dead_marker,x_old,y_old,0,memo,limit)
               #berechnet alte Distanz zu Ziel von alter Pos
                if action!=5:
                   ...
                   #Agent haette Bombe legen koennen, dieser hat dies aber nicht getan
                    if distanz_old<=2 and ((target_x,target_y) in Bomb_Exlpoison_Radius_Local) and (memo[(x_old,y_old,0)]<=limit) and bombs_left>=1:
                           custom_events.append(SHOULD_HAVE_PLANTED_BOMB)
                    #Ziel nicht zu nah und Weg ex. ?
                    elif distanz_old>1 and distanz_old!=1e9:
                        ...
                        if action in possible_good_actions:
                                custom_events.append(STEPPED_TOWARDS_TARGET_POS)
                
                        else :                                                 
                                custom_events.append(STEPPED_TOWARDS_TARGET_NEG)
               
                else:

                    #Agent hat verucht Bombe zu legen ohne eine zu haben?
                    if bombs_left==0:
                         custom_events.append(PLANTED_WITHOUT_BOMBS_LEFT)
                     #Ziel nicht in Reichweite der Bombe
                    if (target_x,target_y) not in Bomb_Exlpoison_Radius_Local:
                        custom_events.append(Target_IN_RANGE_NEG)
                      #Agent kann nicht fliehen
                    if memo[(x_old,y_old,0)]>limit:
                         custom_events.append(CAN_ESCAPE_OWN_BOMB_NEG)
                    #Agent hat Bombe gelegt, Ziel ist in Reichweite und Agent kann fliehen?
                    if ( ((target_x,target_y)  in Bomb_Exlpoison_Radius_Local)) and ( (memo[(x_old,y_old,0)]<=limit)):
                         custom_events.append(CAN_ESCAPE_OWN_BOMB_POS)
                         custom_events.append(Target_IN_RANGE_POS)
    ...
    def _shape_reward(self, events: list[str]) -> float:
        ...
        reward_mapping = {
            ...
            STEPPED_TOWARDS_TARGET_POS:1,
            STEPPED_TOWARDS_TARGET_NEG:-1,
            Target_IN_RANGE_NEG:-3,
            Target_IN_RANGE_POS: 0,
            CAN_ESCAPE_OWN_BOMB_POS:3,
            CAN_ESCAPE_OWN_BOMB_NEG:-10,
            SHOULD_HAVE_PLANTED_BOMB:-3,
            PLANTED_WITHOUT_BOMBS_LEFT:-10
